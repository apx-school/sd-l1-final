...
function ejecutarComando(controller, params) {
  if (params._[0] == "add") {
    ...
    .then((respuesta) => { return respuesta; });
      ...
      return controller.get({ search:{title:  params.tag} })
      .then((respuesta) => { return respuesta; });
    } else if ({}) {
...
function main() {
  ...
  const resultado = ejecutarComando(controller, params)

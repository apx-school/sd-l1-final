...
test.serial(
  ...
  async (t) => {
    ...
    const peli = await controller.getOptions({ id: 4321865 });
...
test.serial("Testeo PelisController get id", async (t) => {
  ...
  const peli = await controller.getOptions({ id: TEST_ID });
...
test.serial("Testeo PelisController search title", async (t) => {
  ...
  const pelis = await controller.getOptions({ search: { title: TEST_ID.toString() } });
  t.is(pelis.length, 1);
...
test.serial("Testeo PelisController search tag", async (t) => {
  ...
  const pelis = await controller.getOptions({
